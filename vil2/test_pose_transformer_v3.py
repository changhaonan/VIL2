...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        pred_pose9d_coarse, pred_status_coarse, pred_pose9d_fine, pred_status_fine = tmorp_model.predict(
            batch=sample_batch
        )

        # Rank the prediction by status
        sorted_indices = np.argsort(pred_status_coarse[:, 1])
        sorted_indices = sorted_indices[::-1]
        pred_pose9d_coarse = pred_pose9d_coarse[sorted_indices]
        pred_status_coarse = pred_status_coarse[sorted_indices]
        ...
        for j in range(sample_size):
            print(f"Status: {pred_status_coarse[j]} for {j}-th sample")
            vis_list = [anchor_pcd]
            # Crop fixed
        ...
        for j in range(3):
            ...
            recover_pose = tmorp_model.pose_recover_rpdiff(
                pred_pose9d_coarse[j], samples[sorted_indices[j]]["crop_center"], data
            )
            # DEBUG:
