...
from vil2.external.rpdiff.utils import util, config_util

from vil2.external.rpdiff.robot.multicam import MultiCams
from vil2.external.rpdiff.robot.floating_sphere_gripper import FloatingSphereGripper
from vil2.external.rpdiff.utils import path_util
from vil2.external.rpdiff.share.globals import (
    bad_shapenet_mug_ids_list,
    bad_shapenet_bowls_ids_list,
    bad_shapenet_bottles_ids_list,
)
from vil2.external.rpdiff.utils.pb2mc.pybullet_meshcat import PyBulletMeshcat
from vil2.external.rpdiff.utils.mesh_util import inside_mesh, three_util
from vil2.external.rpdiff.utils.eval_gen_utils import (
...
def check_ckpt_load_latest(ckpt_path: str) -> str:
    ...
    if not ckpt_path.endswith(".pth"):
        ...
        model_names = [
            val.replace(".pth", "")
            for val in os.listdir(ckpt_path)
            if (("latest" not in val) and (val.endswith(".pth")))
        ]
...
def pb2mc_update(
    recorder: PyBulletMeshcat, mc_vis: meshcat.Visualizer, stop_event: threading.Event, run_event: threading.Event
) -> None:
    iters = 0
    # while True:
...
def main(args: config_util.AttrDict) -> None:
    ...
    pb_client = create_pybullet_client(
        gui=args.experiment.pybullet_viz, opengl_render=True, realtime=True, server=args.experiment.pybullet_server
    )
    ...
    for k, v in mesh_data_dirs.items():
        ...
        objects_filtered = [
            ...
            if (fn.split("/")[-1] not in bad_ids[k]) and ("dec" not in fn and fn.endswith(".obj"))
        ]
    ...
    assert (
        args.experiment.eval.parent_load_pose_type in valid_load_pose_types
    ), f'Invalid string value for args.experiment.eval.parent_load_pose_type! Must be in {", ".join(valid_load_pose_types)}'
    assert (
        args.experiment.eval.child_load_pose_type in valid_load_pose_types
    ), f'Invalid string value for args.experiment.eval.child_load_pose_type! Must be in {", ".join(valid_load_pose_types)}'
    ...
    if osp.exists(parent_test_split_fname):
        ...
        pc_master_dict["parent"]["test_ids"] = [
            val for val in sorted(mesh_names[parent_class])[train_np:] if ("_dec" not in val) and (val.endswith(".obj"))
        ]
    if osp.exists(child_test_split_fname):
        ...
        pc_master_dict["child"]["test_ids"] = [
            val for val in sorted(mesh_names[child_class])[train_nc:] if ("_dec" not in val) and (val.endswith(".obj"))
        ]
    ...
    for iteration in range(exp_args.start_iteration, exp_args.num_iterations):
        ...
        parent_id_list = random.sample(
            pc_master_dict["parent"]["test_ids"], np.random.randint(1, exp_args.n_parent_instances + 1)
        )
        ...
        if eval_cabinet_task:
            ...
            saved_available_poses_fname = osp.join(
                parent_mesh_dir, "open_slot_poses", cabinet_name + "_open_slot_poses.npz"
            )
        ...
        if is_parent_shapenet_obj:
            parent_obj_file_list = [
                osp.join(mesh_data_dirs[parent_class], parent_id, "models/model_normalized.obj")
                for parent_id in parent_id_list
            ]
            parent_obj_file_dec_list = [
                parent_obj_file.split(".obj")[0] + "_dec.obj" for parent_obj_file in parent_obj_file_list
            ]
        else:
            parent_obj_file_list = [
                osp.join(mesh_data_dirs[parent_class], parent_id + ".obj") for parent_id in parent_id_list
            ]
            parent_obj_file_dec_list = [
                parent_obj_file.split(".obj")[0] + "_dec.obj" for parent_obj_file in parent_obj_file_list
            ]
        ...
        if is_child_shapenet_obj:
            child_obj_file_list = [
                osp.join(mesh_data_dirs[child_class], child_id, "models/model_normalized.obj")
                for child_id in child_id_list
            ]
            child_obj_file_dec_list = [
                child_obj_file.split(".obj")[0] + "_dec.obj" for child_obj_file in child_obj_file_list
            ]
        else:
            child_obj_file_list = [
                osp.join(mesh_data_dirs[child_class], child_id + ".obj") for child_id in child_id_list
            ]
            child_obj_file_dec_list = [
                child_obj_file.split(".obj")[0] + "_dec.obj" for child_obj_file in child_obj_file_list
            ]
        ...
        for pc in pcl:
            ...
            for obj_ind, obj_file in enumerate(obj_file_list):
                ...
                pc_master_dict[pc]["mesh_file_dec"] = (
                    parent_obj_file_dec_list[obj_ind] if pc == "parent" else child_obj_file_dec_list[obj_ind]
                )
        ...
        parent_pcd_original = copy.deepcopy(parent_pcd)
        ...
        if exp_args.eval.load_full_pcd:
            ...
            # obj_obj_file, obj_obj_file_dec = pc_master_dict[pc]['mesh_file'], pc_master_dict[pc]['mesh_file_dec']
            child_pcd_orig = (
                trimesh.load(child_obj_file_list[0], process=False).apply_scale(custom_book_scale).sample(5000)
            )
            child_pcd = util.transform_pcd(child_pcd_orig, util.matrix_from_list(child_obj_pose_world))
        else:
            child_pcd = pc_obs_info["pcd"]["child"]
        child_pcd_original = copy.deepcopy(child_pcd)
        ...
        if (
            eval_bookshelf_task or eval_cabinet_task or eval_mug_rack_multi_task
        ) and exp_args.use_floating_hand_execution:
        ...
        if not exp_args.use_floating_hand_execution:
            ...
            pb_client.reset_body(
                place_parent_obj_id, parent_upside_down_pose_list[:3], parent_upside_down_pose_list[3:]
            )
            ...
            pb_client.reset_body(
                child_obj_id, final_child_pose_upside_down_list[:3], final_child_pose_upside_down_list[3:]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config_fname",
        type=str,
        default="can_on_cabinet/can_on_cabinet_withsc.yaml",
    ...
    parser.add_argument(
        "-ex", "--export_viz", action="store_true", help="If True, save data for post-processed visualization"
    )
    parser.add_argument("--export_viz_dirname", type=str, default="rpdiff_export_viz")
    parser.add_argument(
        "-cc", "--compute_coverage", action="store_true", help="If True, save data for post-processed visualization"
    )
    ...
    eval_args = config_util.load_config(
        osp.join(path_util.get_eval_config_dir(), args.config_fname), demo_train_eval="eval"
    )
