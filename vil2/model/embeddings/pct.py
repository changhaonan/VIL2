...
class Local_op(nn.Module):
    """Aggregate feature within local region"""
    ...
    def forward(self, x):
        b, n, s, d = x.size()  # torch.Size([32, 512, 32, 6])  # Batch, npoint, nsample, channels
        x = x.permute(0, 1, 3, 2)
        x = x.reshape(-1, d, s)
...
class StackedAttention(nn.Module):
    """Two layers of self-attention with residual connection"""
...
class PointTransformerEncoderSmall(nn.Module):
    def __init__(self, output_dim=256, input_dim=6, points_pyramid=[128, 32], mean_center=True):
        super(PointTransformerEncoderSmall, self).__init__()

        self.mean_center = mean_center
        self.points_pyramid = points_pyramid
        ...
        self.relu = nn.ReLU()
        self.conv_fuse = nn.Sequential(
            nn.Conv1d(192, output_dim, kernel_size=1, bias=False),
    ...
    def pct(self, x):
        """Hierarchical Point-Cloud Transformer"""
        # x: B, N, D
        xyz = x[..., :3]
        x = x.permute(0, 2, 1)
        batch_size, _, _ = x.size()
        x = self.relu(self.bn1(self.conv1(x)))  # B, D, N
        x = x.permute(0, 2, 1)
        new_xyz, new_feature = sample_and_group(npoint=self.points_pyramid[0], nsample=32, xyz=xyz, points=x)
        feature_0 = self.gather_local_0(new_feature)
        feature = feature_0.permute(0, 2, 1)  # B, nsamples, D
        new_xyz, new_feature = sample_and_group(npoint=self.points_pyramid[1], nsample=16, xyz=new_xyz, points=feature)
