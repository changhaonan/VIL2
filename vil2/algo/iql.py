...
class IQL(OfflinePolicy):
    ...
    def __init__(self, env: gym.Env, dataset: dict, config: dict):
        ...
        self.policy = PolicyNetwork(input_dim=policy_input_dim, hidden_dim=policy_hidden_dim,
                                    output_dim=policy_output_dim, is_gaussian=policy_is_gaussian)
    ...
    def train(self, env, batch_size: int, num_epochs: int, eval_period: int, alpha: float = 0.01, tau: float = 0.6, gamma: float = 0.99, beta: float = 3.0, update_action: bool = False):
        ...
        for epoch in range(num_epochs):
            ...
            batch_idx = torch.randint(low=0, high=self.observations.shape[0], size=(
            ...
            # update action
            if update_action:
                self.update_policy(observations=observations,
                                   actions=actions, beta=beta)

            # eval
            v_loss_sum += vf_loss.detach().cpu().numpy()
            q_loss_sum += (qf1_loss.detach().cpu().numpy() +
                           qf2_loss.detach().cpu().numpy()) / 2.0
            # print(f"Epoch: {epoch}, V Loss: {v_loss_sum / eval_period}, Q Loss: {q_loss_sum / eval_period}")
            if epoch % eval_period == 0:
                if update_action:
                    self.evaluate(env=env, num_epochs=10,
                                  epoch=epoch, enable_render=self.render_eval)
                # statics
                v_loss_sum = 0.0
                q_loss_sum = 0.0
    ...
    def extract_policy(self, batch_size: int = 256, num_epochs: int = 1000, beta: float = 3.0):
        ...
        for epoch in range(num_epochs):
            ...
            actions = self.actions[batch_idx]
            # update policy
            pi_loss = self.update_policy(
                observations=observations, actions=actions, beta=beta)
            # eval
            if epoch % 10 == 0:
    ...
    def update_policy(self, observations, actions, beta: float = 1.0):
        ...
        q_target_pred = torch.min(self.target_qf1(torch.cat(
            [observations, actions], dim=1)), self.target_qf2(torch.cat([observations, actions], dim=1)))
        advantages = q_target_pred - self.vf(observations)
        # don't update V & Q network with policy loss
        advantages_exp = torch.exp(
            advantages * beta).detach().clamp(max=EXP_ADV_MAX)
        if self.action_discrete:
            action_mean = self.policy(observations)
            # use cross entropy loss
            log_probs = - \
                nn.CrossEntropyLoss(reduction='none')(
                    action_mean, actions.argmax(dim=-1))
        else:
        ...
        with torch.no_grad():
            ...
            if self.action_discrete:
                ...
                if self.policy.is_gaussian:
                    with torch.no_grad():
                        action_mean = self.policy(observations)
                        action_std = torch.ones_like(
                            action_mean) * self.policy_std
                        action_dist = torch.distributions.Normal(
                            action_mean, action_std)
